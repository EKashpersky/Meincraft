...
ChunkBlock World::getBlock(int x, int y, int z) {
  ...
  return m_chunkManager.getChunk(chunkPosition.x, chunkPosition.z).getBlock(
    bp.x, y, bp.z);
}
...
void World::setBlock(int x, int y, int z, ChunkBlock block) {
  ...
  m_chunkManager.getChunk(chunkPosition.x, chunkPosition.z).setBlock(bp.x, y,
                                                                     bp.z,
                                                                     block);
}
...
void World::updateChunk(int blockX, int blockY, int blockZ) {
  ...
  addChunkToUpdateBatch(key, m_chunkManager.getChunk(chunkPosition.x,
                                                     chunkPosition.z).getSection(
    chunkSectionY));
  ...
  if (sectionBlockXZ.x == 0) {
    ...
    addChunkToUpdateBatch(newKey, m_chunkManager.getChunk(newKey.x,
                                                          newKey.z).getSection(
      newKey.y));
...
void World::setSpawnPoint() {
  ...
  while (blockY <= h) {
    ...
    blockY = m_chunkManager.getChunk(chunkX, chunkZ).getHeightAt(blockX,
                                                                 blockZ);
    attempts++;
  }
